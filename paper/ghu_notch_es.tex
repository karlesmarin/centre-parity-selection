...
$\{1,-1,t,t^{-1}\}$ tiene determinante $-1$, de modo que parametriza la componente no identidad
...
\emph{Establecido aquí:} que en este orbifold el desplazamiento unidad $\alpha_2\to\alpha_2+1$,
que por sí solo \emph{no} es central, es Weyl-equivalente al elemento central $-\mathbf{1}$,
aportando el twist $U_6$ el signo compensador --- de modo que la gradación estándar por
centro y $n$-alidad de un potencial de holonomía~\cite{Weiss,GPY,RobergeWeiss,UnsalYaffe}
gobierna este modelo, donde no es manifiesta, y la carga del centro pasa a ser un enunciado
sobre el potencial y no solo sobre la materia. \emph{Abierto:} sus cuatro lecturas, que son el
resto del trabajo.
...
Los datos de contorno son los de AHMN, reunidos en la Figura~\ref{fig:fundamental}: traslaciones $U_{5,6}$, paridades $P_i$ en los cuatro
...
Las dos ramas no están al mismo nivel y las separamos. La primera, $a,b,c$ todos impares, dice
que todo $\mu_j$ tiene la misma paridad, con lo que las dos filas constantes del bialternante se
vuelven proporcionales; es la obstrucción clásica de paridad del $\beta$-conjunto y no tiene
nada de especial de nuestro alfabeto --- es exactamente el lugar de ceros de
$s_\lambda(1,-1,y_1,y_2)$ con $y_1,y_2$ \emph{libres}. Debe leerse como estándar. La segunda,
$a=c$ con $b$ impar, no es una identidad de funciones simétricas en variables libres: liberada
de $y_2=y_1^{-1}$ deja de anularse, y sobre las etiquetas hasta $5$ los dos lugares difieren en
exactamente las seis formas $(0,1,0)$, $(0,3,0)$, $(2,1,2)$, $(2,3,2)$, $(4,1,4)$, $(4,3,4)$,
todas de esta rama. Vive sobre el coset ortogonal y en ningún otro sitio, y por eso su
enunciado natural es el de la \S\ref{sec:reflections}: el carácter de $GL(4)$ se anula
idénticamente sobre $O(4)^-$, equivalentemente
$\mathrm{Res}\,V_\lambda\cong\mathrm{Res}\,V_\lambda\otimes\det$.

Un antecedente merece ser nombrado para esta rama en concreto. El teorema de factorización de
Ayyer y Behrend~\cite[Tma.~1]{AyyerBehrend} se enuncia para particiones con
$\lambda_1+\lambda_4=\lambda_2+\lambda_3=2k$, que para cuatro filas es exactamente $a=c$, y su
caso de semipartición es exactamente $b$ impar --- su hipótesis de forma es nuestra rama, en su
subcaso más delicado. Su alfabeto es $(x_1,x_2,\bar x_1,\bar x_2)$, la componente identidad, de
modo que el teorema no alcanza al nuestro, y su prefactor $\prod(x_i^{1/2}+\bar x_i^{1/2})$
degenera precisamente en $x_i=-1$. No reclamamos más que esto: la familia es suya y el coset es
nuestro.
...
genérico de la componente no identidad (\emph{impropia}) $O(4)^-$; una representación irreducible de $O(4)$
...
componente no identidad $\chi^-_{(x-1)}$ salvo normalización. Damos la ruta del bialternante
...
en etiquetas de Dynkin y su certificado en Lean, no el fenómeno ni la técnica. El valor del
carácter en una involución $(1^p,(-1)^q)$, que nuestra vía por el bialternante reproduce de
camino, tampoco es nuestro: es Karmakar~\cite[Tma.~4.1]{Karmakar}, con el mismo dispositivo de
corte por paridad de $\lambda+\rho$ y la misma demostración de resta de columnas seguida de
Laplace.

\emph{Establecido aquí:} $\mathcal Z$ exactamente, disjunta de la clase admisible de la
Parte~II.

\paragraph{Cuatro preguntas que este artículo no puede responder.} El Teorema~\ref{thm:Z} dice
exactamente \emph{cuándo} se anula el carácter sombra, y el resto del artículo gasta ese hecho.
Deja cuatro cosas en pie, y las enunciamos como preguntas y no como lamentos porque son
contestables y porque la respuesta resultó ser un solo objeto:

\begin{enumerate}\itemsep2pt
\item[\textbf{P1}] ¿Es $\mathcal Z$ corolario de algún criterio general para que un carácter de
$GL(n)$ se anule en la componente no identidad, o es particular de $\su4$?
\item[\textbf{P2}] El teorema dice dónde $D$ es cero. \emph{¿Qué es $D$ cuando no lo es?} Usamos
sus coeficientes de principio a fin y nunca sabemos a qué función pertenecen.
\item[\textbf{P3}] ¿Por qué exactamente dos ramas? La Figura~\ref{fig:landscape} no muestra una
tercera causa de anulación, y no tenemos razón para que no pudiera haberla.
\item[\textbf{P4}] Los momentos $\sum_m m^{2r}\delta(m)$ deciden todo lo que sobrevive a
\S\ref{sec:decoupling}, y los obtenemos enumerando el sistema de pesos de cada irreducible.
¿Hay estructura ahí, o la enumeración es la única vía?
\end{enumerate}

\emph{Contestadas en otro sitio (v4).} Las cuatro se resuelven en la Parte~IV, y con un solo
enunciado: una forma cerrada para $s_\lambda(1,-1,t,t^{-1})$ como $\pm$ un producto de
exactamente tres caracteres de $SU(2)$ leídos del $2$-cociente de $\lambda$, o cero. \textbf{P1}
sí, y por algo más fuerte que un criterio; \textbf{P2} ese producto; \textbf{P3} porque un
producto de tres caracteres tiene exactamente dos maneras de anularse; \textbf{P4} hay
estructura --- $\delta$ es una convolución triple, así que todo momento es polinómico en los
tres índices y la enumeración sobra. Eso es un enunciado sobre funciones simétricas, y se sitúa junto al programa de
factorización de Ciucu--Krattenthaler y Ayyer--Behrend~\cite{CiucuKrattenthaler,AyyerBehrend}
sin pertenecer a él ni a la línea de raíces de la unidad. La Parte~IV lo desarrolla; lo que
devuelve a la física es el asunto del párrafo siguiente.

Conviene decir qué vuelve en sentido contrario, porque el intercambio no es unilateral. La
forma cerrada convierte $D$ en un producto de exactamente tres caracteres de $SU(2)$, de modo
que $\delta(m)$ es una convolución triple y sus momentos $\sum_m m^{2r}\delta(m)$ resultan
\emph{polinómicos} en los tres índices. Esos momentos son justamente las cantidades que este
artículo tuvo que extraer enumerando el sistema de pesos de cada irreducible, y son los que
deciden la comparación entre representaciones admisibles una vez que \S\ref{sec:decoupling} ha
eliminado el término dominante. La pregunta que exportamos a la Parte~IV como curiosidad sobre
funciones simétricas vuelve como el asidero analítico de la estructura subdominante que aquí
podíamos describir y no calcular.
...
\begin{quote}\itshape
``For even $m$, the combination $(m\alpha_2+q)/2$ is invariant under
$\alpha_2\to\alpha_2+1$ up to an integer shift, and hence each such tower is periodic with
period 1 in the $\alpha_2$ direction. For odd $m$, the spectrum with $(m,q)=(m,1)$ at a given
$\alpha_2$ is equivalent to that with $(m,q)=(m,0)$ at $\alpha_2+1$. Hence, if the effective
potential contains an equal number of KK mode with $(m,0)$ and $(m,1)$ for odd $m$, their
combined contribution is also periodic under $\alpha_2\to\alpha_2+1$.''
\end{quote}
Citamos también la frase de $m$ par porque juega a favor del argumento y no en contra: las
torres de $m$ par son periódicas de período $1$ por sí solas, sea cual sea la representación,
de modo que el sector de $m$ impar es el \emph{único} lugar donde la periodicidad puede
obstruirse --- y por eso una representación que falle ahí pierde la reducción por completo.
...
la carga del centro. La Tabla~\ref{tab:flip} es el argumento entero.\footnote{Que el período
dependa de la representación no es tanto invisible en la literatura previa como no comentado
en ella: la fórmula general en cinco dimensiones de Haba, Takenaga y Yamashita~\cite{HTY} ya
lleva las contribuciones de tipo adjunto como $\cos(2\pi n a)$ frente a las de tipo fundamental
como $\cos(\pi n a)$, períodos que difieren exactamente en el factor dos en cuestión. La
distinción está presente en sus ecuaciones y se obtiene como consecuencia de las sumas de
modos; no la hemos encontrado enunciada en ninguna parte como regla sobre qué armónicos pueden
existir, y ese es el hueco que llena la carga del centro.}
...
las ocho representaciones admisibles de dimensión $\le756$.
...
\textcolor{white}{$\mathbf{224}$} & \textcolor{white}{$\mathbf{280}$} & \textcolor{white}{$\mathbf{360}$} &
\textcolor{white}{$\mathbf{756}$}\\
\midrule
$\max|V(\alpha_2{+}1)-V|/\text{escala}$ & $\mathbf{0}$ & $0.30$ & $0.21$ & $0.045$ & $0.12$ &
$0.19$ & $0.12$ & $0.095$ & $0.023$\\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Relación con el programa de clases de equivalencia de condiciones de contorno.}
Existe una literatura desarrollada sobre redundancia gauge en exactamente este marco, y
conviene decir con precisión por qué no contiene el enunciado anterior. Kawamura y
colaboradores, y más recientemente Takeuchi e Inagaki, clasifican \emph{clases de
equivalencia de condiciones de contorno} en $S^1/\ZZ_2$ y $T^2/\ZZ_m$ bajo las
transformaciones gauge que las conectan~\cite{HHK,KawamuraMiura,Takeuchi1,Takeuchi2}. Su
invariante es una ley de conservación de la traza: $\mathrm{tr}\,P_i$ se preserva en cada
punto fijo, de modo que las degeneraciones de autovalores de las matrices de condiciones de
contorno no pueden cambiar dentro de una clase.

Es un invariante distinto sobre un objeto distinto. El suyo se evalúa sobre las
\emph{matrices de condiciones de contorno} $N\times N$ y restringe qué condiciones discretas
pueden identificarse; el nuestro se evalúa sobre una \emph{representación irreducible}
arbitraria a condiciones de contorno fijas, y restringe qué armónicos del potencial pueden ser
no nulos. Ninguno calcula el otro.

Una ecuación de ese programa sí merece ser nombrada como antecedente, y la nombramos. Haba,
Hosotani y Kawamura exhiben, para $SU(2)$ y como ilustración de que las condiciones de contorno
se mueven bajo transformaciones gauge, la equivalencia
$(\tau_3,\tau_3,\mathbf{1})\sim(\tau_3,-\tau_3,-\mathbf{1})$~\cite[Ec.~(2.20)]{HHK}, obtenida
de $\Omega=\exp\{i(\alpha y/2\pi R)\tau_2\}$ en $\alpha=\pi$ --- que en la normalización
empleada aquí es exactamente un período de línea de Wilson. Esa es la sombra $N=2$ de
\eqref{eq:principle}, y a quien la conozca hay que decirle por qué no contiene ya el enunciado.
En $SU(2)$ el bloque \emph{es} el grupo, así que la holonomía $-\mathbf{1}$ es central de balde
y no hace falta compensación alguna. Embebida en $SU(N)$ a lo largo de su propia
relación~\cite[Ec.~(2.21)]{HHK}, esa misma matriz pasa a ser $\mathrm{diag}(\dots,-1,-1,\dots)$,
que no es central, y de ahí no se sigue ningún enunciado sobre el centro. Lo que hace funcionar
a \eqref{eq:principle} en $N=4$ es el twist, no el desplazamiento; ese es el paso sin
contrapartida allí. Coherentemente, las palabras \emph{centro}, \emph{Weyl}, \emph{período} y
\emph{$n$-alidad} no aparecen en \cite{HHK}, y su potencial efectivo se calcula únicamente a
fases de línea de Wilson nulas. Dos rasgos de ese programa hacen la separación nítida y no
una cuestión de énfasis: declina calcular potencial efectivo alguno (\emph{``we do not carry
them out since our purpose is not to study the dynamics of models but to classify the
BCs''}~\cite[p.~7]{KawamuraMiura}), y señala que la traza de la matriz de \emph{traslación}
no se conserva en general~\cite[n.~3]{Takeuchi1} --- siendo el sector de traslación
precisamente donde vive nuestro desplazamiento $\alpha_2\to\alpha_2+1$.
...
$\mathbf{35}$, $\mathbf{60}$, $\mathbf{84}$, $\mathbf{280}$ y $\mathbf{756}$
...
\end{equation}
Nada de \eqref{eq:Dstruct} es nuevo, y conviene ser preciso al respecto antes de extraer la
consecuencia. Es el término $(k_1,k_2)=(0,\pm1)$ de la suma reticular de Coleman--Weinberg
estándar derivada dos veces en $\alpha_2=\tfrac12$, donde la fase
$\cos\big(\pi k_2(\tfrac m2+q)\big)$ mata los $m$ impares y deja $(-1)^{q+m/2}$ en el resto; la
maniobra de sustituir el valor especial de la línea de Wilson, mirar qué clase de resto se
cancela y leer el signo está escrita en Hosotani, Noda y
Takenaga~\cite[Ecs.~(6.5)--(6.7)]{HNT} para este mismo orbifold y en Haba, Hosotani, Kawamura y
Yamashita~\cite[Ec.~(46)]{HHKY}. Tampoco la gradación módulo cuatro es tan estructural como
parece: sustituyendo $m=2m'$ colapsa a un peso alternante corriente,
\[
\mathcal D=-4\pi^2\sum_{m'}(-1)^{m'}m'^2\,\delta(2m'),
\]
de modo que \eqref{eq:Dstruct} es la curvatura estándar de winding líder escrita en clases de
resto, y así debe leerse. Lo que no se sigue de nada de eso es el corolario, porque la muesca
es precisamente el enunciado $\delta(m)=0$ para todo $m$ par. Por tanto:
...
\rowcolor{softgrey}
$\partial^2_{\alpha_2}V$ real en $\tfrac12$ & $-200.6$ & $+21.2$ & $-5.8$ & $-85.1$ & $-188.6$ & $+16.8$ & $+255.2$ & $-160.4$ & $+245.5$\\
el signo coincide & sí & \textbf{no} & sí & sí & sí & \textbf{no} & \textbf{no} & sí & \textbf{no}\\
\bottomrule
\end{tabular}
\end{center}
...
la clase de congruencia como invariante que gobierna la ruptura por líneas de Wilson, en el
propio modelo de Hosotani~\cite{DaviesMcLachlan1,DaviesMcLachlan2} &
nada --- como en la Parte~II, no reclamamos nada de ello\\
\midrule
\rowcolor{softgrey}
orbifold, twists $(m,q)$, potencial a un loop, Ec.~\eqref{eq:region} &
AHMN~\cite{AHMN,AHMN23}; los coeficientes $\{A+B(-1)^{k_2}\}$ para la $\mathbf{35}$ son
\cite[Ec.~(3.25)]{AHMN23} &
...
\emph{Actualizado en la v4.} El libro de Littlewood, Cap.~XI, ha sido obtenido y leído desde
entonces: contiene el mecanismo autoasociado (\S11.6, pp.~226--227; \S11.8, p.~235), y por eso
lo citamos, pero no contiene el criterio sobre el peso $\lambda$ de $GL(n)$. El trabajo de King
de 1971 en su propia exposición sigue siendo inaccesible y sigue señalado como no leído; su
resumen y una reproducción fiel en otro lugar indican que aporta la herramienta, no el teorema.
Y la búsqueda registrada arriba era ella misma deficiente: se ejecutó con una herramienta
bibliográfica cuyo constructor de consultas descartaba en silencio los términos discriminantes,
de modo que un resultado nulo suyo no certificaba nada. La repetición, ya corregida y con
INSPIRE-HEP y zbMATH Open añadidos, es lo que ha producido las citas a
\cite{DaviesMcLachlan1,DaviesMcLachlan2,Karmakar,AHMN23} que esta versión incorpora.

% ===================================================================
\section{Reflexiones}\label{sec:reflections}
\paragraph{Un nulo de una herramienta de búsqueda no es un hueco (v4).} La versión anterior de
esta sección registraba, con razón, que habíamos buscado un criterio general de componente
no identidad y no lo habíamos hallado. Lo que no registraba --- porque no lo sabíamos --- es que la
herramienta bibliográfica empleada en parte de esa búsqueda era defectuosa de un modo que solo
podía producir silencio: su constructor de consultas conservaba los primeros términos por
posición y no por especificidad, de manera que los términos discriminantes se descartaban antes
de buscar, y varias fuentes no estaban instaladas, dejando invisible todo lo publicado antes
de~1991. Una herramienta en ese estado devuelve pocas referencias o ninguna, y pocas referencias
o ninguna se lee exactamente igual que novedad. Dos de las citas que esta versión añade,
\cite{DaviesMcLachlan1,DaviesMcLachlan2}, son de 1988 y 1989, superan las cien citas cada una, y
aparecen como los dos primeros resultados de la primera consulta en cuanto la herramienta
funciona. Lo dejamos escrito no como disculpa sino como punto metodológico con dientes:
\emph{un resultado negativo de búsqueda es evidencia solo en proporción a la sensibilidad
demostrada de esa búsqueda}, y esa sensibilidad debe establecerse con una consulta cuya
respuesta ya se conoce, antes de creerse ningún nulo. La misma disciplina que este artículo
aplica a las inferencias físicas se aplica a las bibliográficas, y no la estábamos aplicando.
...
\bibitem{AHMN23} K.~Akamatsu, T.~Hirose, N.~Maru, A.~Nago, \emph{Electroweak Symmetry Breaking
in Two Higgs Doublet Model from 6D Gauge--Higgs Unification on $T^2/\ZZ_2$},
arXiv:2312.08608.
\bibitem{DaviesMcLachlan1} A.T.~Davies, A.~McLachlan, \emph{Gauge group breaking by Wilson
loops}, Phys.\ Lett.\ B \textbf{200} (1988) 305.
\bibitem{DaviesMcLachlan2} A.T.~Davies, A.~McLachlan, \emph{Congruency class effects in the
Hosotani model}, Nucl.\ Phys.\ B \textbf{317} (1989) 237.
...
arXiv:0812.1251.
\bibitem{HTY} N.~Haba, K.~Takenaga, T.~Yamashita, \emph{A general formula of the effective
potential in 5D $SU(N)$ gauge theory on orbifold}, arXiv:hep-ph/0401185.
\bibitem{HNT} Y.~Hosotani, S.~Noda, K.~Takenaga, \emph{Dynamical gauge symmetry breaking and
mass generation on the orbifold $T^2/Z_2$}, arXiv:hep-ph/0403106.
\bibitem{HHKY} N.~Haba, Y.~Hosotani, Y.~Kawamura, T.~Yamashita, \emph{Dynamical symmetry
breaking in gauge-Higgs unification on orbifold}, arXiv:hep-ph/0401183.
\bibitem{Weiss} N.~Weiss, \emph{Effective potential for the order parameter of gauge theories
at finite temperature}, Phys.\ Rev.\ D \textbf{24} (1981) 475.
\bibitem{GPY} D.~J.~Gross, R.~D.~Pisarski, L.~G.~Yaffe, \emph{QCD and instantons at finite
temperature}, Rev.\ Mod.\ Phys.\ \textbf{53} (1981) 43.
\bibitem{RobergeWeiss} A.~Roberge, N.~Weiss, \emph{Gauge theories with imaginary chemical
potential and the phases of QCD}, Nucl.\ Phys.\ B \textbf{275} (1986) 734.
\bibitem{UnsalYaffe} M.~\"Unsal, L.~G.~Yaffe, \emph{Center-stabilized Yang--Mills theory:
confinement and large $N$ volume independence}, Phys.\ Rev.\ D \textbf{78} (2008) 065035,
arXiv:0803.0344.
\bibitem{HHK} N.~Haba, Y.~Hosotani, Y.~Kawamura, \emph{Classification and dynamics of
equivalence classes in $SU(N)$ gauge theory on the orbifold $S^1/Z_2$}, Prog.\ Theor.\ Phys.\
\textbf{111} (2004) 265, arXiv:hep-ph/0309088.
\bibitem{KawamuraMiura} Y.~Kawamura, T.~Miura, \emph{Equivalence Classes of Boundary
Conditions in $SU(N)$ Gauge Theory on 2-dimensional Orbifolds}, arXiv:0905.4123.
\bibitem{Takeuchi1} K.~Takeuchi, T.~Inagaki, \emph{A new classification method for
equivalence classes on $S^1/Z_2$ and $T^2/Z_3$}, PTEP \textbf{2024}, arXiv:2401.09809.
\bibitem{Takeuchi2} K.~Takeuchi, T.~Inagaki, \emph{Trace conservation laws in $T^2/Z_m$
orbifold gauge theories}, arXiv:2404.19411.
