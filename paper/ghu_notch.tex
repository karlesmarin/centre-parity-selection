...
$\{1,-1,t,t^{-1}\}$ has determinant $-1$, so it parametrizes the non-identity component of
...
\emph{Established here:} that in this orbifold the unit shift $\alpha_2\to\alpha_2+1$, which
is \emph{not} central on its own, is Weyl-equivalent to the central element $-\mathbf{1}$,
the $U_6$ twist supplying the compensating sign --- so that the standard centre/$n$-ality
grading of a holonomy potential~\cite{Weiss,GPY,RobergeWeiss,UnsalYaffe} governs this model,
where it is not manifest, and the centre charge becomes a statement about the potential and
not only about the matter. \emph{Left open:} all four of its readings, which are the rest of
the paper.
...
The two branches are not on the same footing and we separate them. The first, $a,b,c$ all odd,
says every $\mu_j$ has the same parity, so the two constant rows of the bialternant become
proportional; it is the classical $\beta$-set parity obstruction and it is not special to our
alphabet at all --- it is exactly the vanishing locus of $s_\lambda(1,-1,y_1,y_2)$ with $y_1,y_2$
\emph{free}. It should be read as standard. The second, $a=c$ with $b$ odd, is not a
symmetric-function identity in free variables: released from $y_2=y_1^{-1}$ it stops vanishing,
and over the labels up to $5$ the two loci differ in exactly the six shapes $(0,1,0)$, $(0,3,0)$,
$(2,1,2)$, $(2,3,2)$, $(4,1,4)$, $(4,3,4)$, all of this branch. It lives on the orthogonal coset
and nowhere else, which is why its natural statement is the one of \S\ref{sec:reflections}:
the $GL(4)$ character vanishes identically on $O(4)^-$, equivalently
$\mathrm{Res}\,V_\lambda\cong\mathrm{Res}\,V_\lambda\otimes\det$.

One ancestor deserves naming for this branch specifically. Ayyer and
Behrend's factorization theorem~\cite[Thm.~1]{AyyerBehrend} is stated for partitions with
$\lambda_1+\lambda_4=\lambda_2+\lambda_3=2k$, which for four rows is exactly $a=c$, and its
half-partition case is exactly $b$ odd --- their shape hypothesis is our branch, in its more
delicate sub-case. Their alphabet is $(x_1,x_2,\bar x_1,\bar x_2)$, the identity component, so
the theorem does not reach ours, and their prefactor $\prod(x_i^{1/2}+\bar x_i^{1/2})$
degenerates precisely at $x_i=-1$. We claim no more than that the family is theirs and the
coset is ours.
...
semisimple element of the non-identity (\emph{improper}) component $O(4)^-$; an $O(4)$ irreducible
...
$V=-2[1]^3$, so the brackets $[x]$ are the non-identity-component characters
...
its Lean certificate, not the phenomenon and not the technique. The character value at an
involution $(1^p,(-1)^q)$, which our bialternant route reproduces along the way, is likewise
not ours: it is Karmakar~\cite[Thm.~4.1]{Karmakar}, with the same parity-split-of-$\lambda+\rho$
device and the same column-subtraction-then-Laplace proof.

\emph{Established here:} $\mathcal Z$ exactly, disjoint from Part~II's admissible class.

\paragraph{Four questions this paper cannot answer.} Theorem~\ref{thm:Z} tells us exactly
\emph{when} the shadow character vanishes, and the rest of this paper spends that fact. It
leaves four things standing, and we state them as questions rather than as regrets because they
are answerable and because the answers turned out to be one object:

\begin{enumerate}\itemsep2pt
\item[\textbf{Q1}] Is $\mathcal Z$ a corollary of some general criterion for a $GL(n)$ character
to vanish on the non-identity component, or is it particular to $\su4$?
\item[\textbf{Q2}] The theorem says where $D$ is zero. \emph{What is $D$ when it is not?} We use
its coefficients throughout and never learn what function they belong to.
\item[\textbf{Q3}] Why exactly two branches? Figure~\ref{fig:landscape} shows no third cause of
vanishing, and we have no reason why there could not be one.
\item[\textbf{Q4}] The moments $\sum_m m^{2r}\delta(m)$ decide everything that survives
\S\ref{sec:decoupling}, and we obtain them by enumerating the weight system of each irreducible.
Is there structure there, or is enumeration the only route?
\end{enumerate}

\emph{Answered elsewhere (v4).} All four are settled in Part~IV, and by a single statement: a
closed form for $s_\lambda(1,-1,t,t^{-1})$ as $\pm$ a product of exactly three $SU(2)$
characters read off the $2$-quotient of $\lambda$, or zero. \textbf{Q1} yes, and by something
stronger than a criterion; \textbf{Q2} that product; \textbf{Q3} because a product of three
characters has exactly two ways to be zero; \textbf{Q4} there is structure --- $\delta$ is a
threefold convolution, so every moment is a polynomial in the three indices, and the enumeration
is unnecessary. That is a statement about symmetric functions, and it sits beside the factorisation
programme of Ciucu--Krattenthaler and Ayyer--Behrend~\cite{CiucuKrattenthaler,AyyerBehrend}
without belonging to it or to the root-of-unity line. Part~IV develops it; what it hands back to
the physics is the subject of the next paragraph.

It is worth saying what comes back the other way, because the exchange is not one-sided. The
closed form makes $D$ a product of exactly three $SU(2)$ characters, so $\delta(m)$ is a
threefold convolution and its moments $\sum_m m^{2r}\delta(m)$ are \emph{polynomial} in the
three indices. Those moments are exactly the quantities this paper had to extract by
enumerating the weight system of each irreducible, and they are what decides the comparison
between admissible representations once \S\ref{sec:decoupling} has removed the leading term.
The question we exported to Part~IV as a curiosity about symmetric functions returns as the
analytic handle on the subdominant structure we could describe here and not compute.
...
\end{quote}
We quote the even-$m$ sentence as well, because it works for the argument rather than against
it: the even-$m$ towers are individually period-$1$ whatever the representation, so the
odd-$m$ sector is the \emph{only} place where the periodicity can be obstructed --- which is
why a representation that fails there loses the reduction outright.
...
with the centre charge. Table~\ref{tab:flip} is the whole argument.\footnote{That the period
depends on the representation is not invisible in the earlier literature so much as
unremarked in it: the general five-dimensional formula of Haba, Takenaga and
Yamashita~\cite{HTY} already carries adjoint-type contributions as $\cos(2\pi n a)$ against
fundamental-type ones as $\cos(\pi n a)$, periods differing by exactly the factor of two at
issue. The distinction is present in their equations and is drawn as a consequence of the
mode sums; we have found it stated nowhere as a rule about which harmonics may exist, and
that is the gap the centre charge fills.}
...
eight admissible representations of dimension $\le756$.
...
\paragraph{Relation to the boundary-condition equivalence-class programme.} There is a
developed literature on gauge redundancy in exactly this setting, and it is worth saying
precisely why it does not contain the statement above. Kawamura and collaborators, and more
recently Takeuchi and Inagaki, classify \emph{equivalence classes of boundary conditions} on
$S^1/\ZZ_2$ and $T^2/\ZZ_m$ under the gauge transformations that connect
them~\cite{HHK,KawamuraMiura,Takeuchi1,Takeuchi2}. Their invariant is a trace conservation
law: $\mathrm{tr}\,P_i$ is preserved at each fixed point, so the eigenvalue degeneracies of
the boundary-condition matrices cannot change within a class.

That is a different invariant on a different object. Theirs is evaluated on the $N\times N$
\emph{boundary-condition matrices} and constrains which discrete boundary conditions may be
identified; ours is evaluated on an arbitrary \emph{irreducible representation} at fixed
boundary conditions, and constrains which harmonics of the potential may be non-zero. The two
do not compute one another.

One equation of that programme does deserve to be named as an antecedent, and we name it.
Haba, Hosotani and Kawamura exhibit, for $SU(2)$ and as an illustration that boundary
conditions move under gauge transformations, the equivalence
$(\tau_3,\tau_3,\mathbf{1})\sim(\tau_3,-\tau_3,-\mathbf{1})$~\cite[Eq.~(2.20)]{HHK},
obtained from $\Omega=\exp\{i(\alpha y/2\pi R)\tau_2\}$ at $\alpha=\pi$ --- which in the
normalization used here is exactly one period of Wilson line. That is the $N=2$ shadow of
\eqref{eq:principle}, and a reader who knows it should be told why it does not already
contain the statement. In $SU(2)$ the block \emph{is} the group, so the holonomy $-\mathbf{1}$
is central for free and no compensation is needed. Embedded in $SU(N)$ along their own
relation~\cite[Eq.~(2.21)]{HHK} the same matrix becomes $\mathrm{diag}(\dots,-1,-1,\dots)$,
which is not central, and no centre statement follows. What makes \eqref{eq:principle} work at
$N=4$ is the twist, not the shift; that is the step with no counterpart there. Consistently,
the words \emph{centre}, \emph{Weyl}, \emph{period} and \emph{$n$-ality} do not occur in
\cite{HHK}, and its effective potential is computed only at vanishing Wilson-line phases. Two features of that programme make the separation sharp rather
than a matter of emphasis: it declines to compute an effective potential at all
(``we do not carry them out since our purpose is not to study the dynamics of models but to
classify the BCs''~\cite[p.~7]{KawamuraMiura}), and it notes that the trace of the
\emph{translation} matrix is not in general conserved~\cite[fn.~3]{Takeuchi1} --- the
translation sector being precisely where our shift $\alpha_2\to\alpha_2+1$ lives.
...
$\mathbf{35}$, $\mathbf{60}$, $\mathbf{84}$, $\mathbf{280}$ and $\mathbf{756}$
...
\end{equation}
Nothing about \eqref{eq:Dstruct} is new, and it is worth being precise about that before
drawing the consequence. It is the $(k_1,k_2)=(0,\pm1)$ term of the standard Coleman--Weinberg
lattice sum differentiated twice at $\alpha_2=\tfrac12$, where the phase
$\cos\big(\pi k_2(\tfrac m2+q)\big)$ kills odd $m$ and leaves $(-1)^{q+m/2}$ on the rest; the
manoeuvre of substituting the special Wilson-line value, watching a residue class cancel and
reading the sign is written out in Hosotani, Noda and Takenaga~\cite[Eqs.~(6.5)--(6.7)]{HNT}
for this very orbifold and in Haba, Hosotani, Kawamura and
Yamashita~\cite[Eq.~(46)]{HHKY}. Nor is the modulus-four grading as structural as it looks:
substituting $m=2m'$ collapses it to a plain alternating weight,
\[
\mathcal D=-4\pi^2\sum_{m'}(-1)^{m'}m'^2\,\delta(2m'),
\]
so \eqref{eq:Dstruct} is the standard leading-winding curvature written in residue classes and
should be read as such. What does not follow from any of that is the corollary, because the
notch is precisely the statement $\delta(m)=0$ for all even $m$. Therefore:
...
\rowcolor{softgrey}
true $\partial^2_{\alpha_2}V$ at $\tfrac12$ & $-200.6$ & $+21.2$ & $-5.8$ & $-85.1$ & $-188.6$ & $+16.8$ & $+255.2$ & $-160.4$ & $+245.5$\\
sign agrees & yes & \textbf{no} & yes & yes & yes & \textbf{no} & \textbf{no} & yes & \textbf{no}\\
\bottomrule
\end{tabular}
\end{center}
...
congruency class as the invariant controlling Wilson-line breaking, in the Hosotani model
itself~\cite{DaviesMcLachlan1,DaviesMcLachlan2} &
nothing --- as in Part~II, we claim none of it\\
\midrule
\rowcolor{softgrey}
orbifold, twists $(m,q)$, one-loop potential, Eq.~\eqref{eq:region} &
AHMN~\cite{AHMN,AHMN23}; the $\{A+B(-1)^{k_2}\}$ coefficients for the $\mathbf{35}$ are
\cite[Eq.~(3.25)]{AHMN23} &
...
$+1$, the \emph{proper} component.

\emph{Updated in v4.} Littlewood's book, Ch.~XI, has since been obtained and read: it contains
the self-associate mechanism (\S11.6, pp.~226--227; \S11.8, p.~235), so we cite it for that,
but it does not contain the criterion on the $GL(n)$ weight $\lambda$. King's 1971 paper in its
own exposition remains unreachable and is still flagged as unread; its abstract and a faithful
reproduction elsewhere indicate it supplies the tool, not the theorem. And the search recorded
above was itself deficient: it was run with a literature tool whose query builder silently
discarded the discriminating terms of every query, so a null result from it certified nothing.
The re-run, with that fixed and with INSPIRE-HEP and zbMATH Open added, is what produced the
citations to~\cite{DaviesMcLachlan1,DaviesMcLachlan2,Karmakar,AHMN23} that this version adds.

% ===================================================================
\section{Reflections}\label{sec:reflections}
\paragraph{A null from a search tool is not a gap (v4).} The previous version of this section
recorded, correctly, that we had looked for a general non-identity-component criterion and not
found one. What we did not record --- because we did not know it --- is that the literature
tool used for part of that search was defective in a way that could only ever produce silence:
its query builder kept the first few tokens of a query by position rather than by specificity,
so the discriminating terms were discarded before the search ran, and several sources were not
installed at all, leaving everything published before~1991 invisible. A tool in that state
returns few or no references, and few or no references reads exactly like novelty. Two of the
citations added in this version, \cite{DaviesMcLachlan1,DaviesMcLachlan2}, are from 1988 and
1989, carry over one hundred citations each, and appear as the first two results of the first
query once the tool works. We record this not as an apology but as a methodological point with
teeth: \emph{a negative search result is evidence only in proportion to the demonstrated
sensitivity of the search}, and the sensitivity must be established on a query whose answer is
already known before any null is believed. The same discipline this paper applies to physical
inferences --- see the next paragraph, and \S\ref{sec:decoupling} --- applies to bibliographic
ones, and we had not been applying it.
